...
public class Parking extends AbstractEntity {
	...
	public String getName() {
		return client_id.toString().trim() + " " + car_id.toString().trim()
				+ " " + parking_place_id.toString().trim();
	}
	...
	public static String getFieldPresent(Field field) {
		...
		if ("id".equals(fieldName)) {
			...
			return "Клиент";
		} else if ("car_id".equals(fieldName)) {
			return "Машина";
		} else if ("parking_place_id".equals(fieldName)) {
			return "Парковочное место";
		} else if ("put_date".equals(fieldName)) {
			return "Время поставки на парковку";
		} else if ("pickup_date".equals(fieldName)) {
