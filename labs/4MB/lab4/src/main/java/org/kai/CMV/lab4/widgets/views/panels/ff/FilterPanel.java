...
public class FilterPanel extends AbstractPanel {
	...
	public void initType() {
		...
		for (String header : ExtendedTable.getHeaders()) {
			_fieldCombo.add(header);
		}
	}
	...
	private void initFilter() {
		...
		_filter = AbstractFilter.getInstance(_filterPlace, SWT.NONE,
				ExtendedTable.getFields()[_fieldCombo.getSelectionIndex()]);
	...
	private void applyFilter() throws IllegalAccessException {
		...
		for (TableItem item : _visualTable.getItems()) {
			Object[] line = (Object[]) item.getData();
			Object fieldValue = line[filterColumn];
